...
while running:
    ...
    # light_timer = 0
    lit_spots.clear()
    list_sprites = all_sprites.sprites()
    list_len = len(list_sprites)
    prev_b = 0
    next_b = 0
    z = 0
    ...
    for sprite in all_sprites:
        ...
        if pressed_keys[K_w]:
            ...
            if (
                hasattr(sprite, "id")
                and sprite.id > 0
                and pygame.sprite.collide_rect(player, sprite)
            ):
                hit_id.append(1)
        if pressed_keys[K_s]:
            sprite.rect.move_ip(0, speed)
        ...
        if (
            ...
            and hasattr(sprite, "id")
            and not hasattr(sprite, "name")
        ):
            ...
            if not pygame.sprite.collide_rect(player, sprite) and dist < 125:
                if sprite.id != 0 and is_build == False:
                    ...
                    sprite.id == 0
                    and is_build == True
                    and selected_block != 0
                    and player.inventory[str(selected_block)] > 0
                ):
                    checked_light_blocks = False
            ...
            if sprite.id == 7:
                ...
                dist_b.append(
                    [
                        (
                            (
                                (player.rect.x - sprite.rect.x) ** 2
                                + (player.rect.y - sprite.rect.y) ** 2
                            )
                            ** 0.5
                        ),
                        REND_DIST,
                ...
                for cor in lit_spots:
                    ...
                    if abs(cor[0] - sprite.rect.x) < REND_DIST + 50:
                        d = (
                            ...
                            + (cor[1] - sprite.rect.y) ** 2
                        ) ** 0.5

                        if d < REND_DIST + 50:
                            dist_b.append([d, cor[2]])
    ...
    for sprite in ui_group:
        ...
        if sprite.type == 1:
            screen.blit(
                small_font.render("Pick Lv" + str(player.pickaxe), True, (255, 255, 0)),
                (sprite.rect.x, sprite.rect.y - 20),
            )
    ...
    screen.blit(
        font.render("Tools and Upgrades", True, (255, 255, 0)), (SCREEN_WIDTH - 200, 0)
    )
