...
class Player(pygame.sprite.Sprite):
    def __init__(self, inventory, hp, pickaxe, axe, sword):
        ...
        self.rect = self.surf.get_rect(
            center=(round(SCREEN_WIDTH / 2), round(SCREEN_HEIGHT / 2))
        )
    ...
    def __init__(self, pos, spawn_chunk, name, data=None):
        ...
        for y in range(CHUNK_SIZE):
            for x in range(CHUNK_SIZE):
                ...
                if data:
                    self.blocks.append(
                        Block((x * 25 + pos[0], y * 25 + pos[1]), data[y][x])
                    )
                else:
                    if self.mirrored_chance == 1:
                        self.blocks.append(
                            Block(
                                (x * 25 + pos[0], y * 25 + pos[1]),
                                chunk_styles[self.chunk_type][y][x],
                            )
                        )
                    else:
                        self.blocks.append(
                            Block(
                                (x * 25 + pos[0], y * 25 + pos[1]),
                                chunk_styles[self.chunk_type][x][y],
                            )
                        )